...
func TestDefaultGraph(t *testing.T) {
	...
	if len(pl.Stages()) != 6 {
		t.Fatalf("stages = %v, want 6", pl.Stages())
	}
	...
	for _, want := range []string{model.StageWebinfo, model.StageTLSDeep, model.StageCrawl, model.StageBanner, model.StagePTR} {
		if !targets[want] {
			t.Fatalf("light edges = %v, missing %s", targets, want)
		}
	}

	for _, s := range []string{model.StageWebinfo, model.StageTLSDeep, model.StageCrawl, model.StageBanner, model.StagePTR} {
		if len(pl[s].Next) != 0 {
